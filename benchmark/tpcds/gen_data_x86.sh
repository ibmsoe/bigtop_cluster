...
nohup spark-shell --master ${SPARK_MASTER} --name dsdgen  --conf spark.executor.extraJavaOptions="-XX:ParallelGCThreads=4 -XX:+AlwaysTenure" --conf spark.rdd.compress=true --conf spark.io.compression.codec=snappy --conf spark.network.timeout=900 --conf spark.serializer=org.apache.spark.serializer.KryoSerializer --conf spark.default.parallelism=320 --conf spark.sql.shuffle.partitions=200 --conf spark.shuffle.consolidateFiles=true --driver-memory 20g --driver-cores 4 --total-executor-cores 160 --executor-cores 5 --executor-memory 18g --jars ${BIGTOP_BENCH_DIR}/deps/spark-sql-perf/target/scala-2.10/spark-sql-perf_2.10-0.3.2.jar -i ${BIGTOP_BENCH_DIR}/tpcds/dsgen.scala >${OUT} 2>&1 &
